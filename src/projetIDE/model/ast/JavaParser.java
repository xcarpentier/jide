...
class JavaParser/*@bgen(jjtree)*/implements JavaParserTreeConstants, JavaParserConstants {/*@bgen(jjtree)*/
  ...
  public static void main(String args[]) {
    ...
    try {
      System.out.println("\nVoici la representation de la classe\n");
...
static final public void jjtreeOpenNodeScope( Node node ) {
	int i=1, j=1;
	String s="";
	/** Un Paquetage */
	if(node instanceof ASTPackageDeclaration) {
		((SimpleNode) node).visibility_token = new Token("");
		((SimpleNode) node).nodeType_token = getToken(1);
		((SimpleNode) node).name_token = getToken(2);
	}
	/** Une Classe ou Une Interface */
	else if(node instanceof ASTTypeDeclaration) {
	    ((SimpleNode) node).visibility_token = getToken(1);
	    ((SimpleNode) node).nodeType_token = getToken(2);
	    ((SimpleNode) node).name_token = getToken(3);
	}
	/** Import */
		...
		((SimpleNode) node).nodeType_token = getToken(1);
		i=2;
		while(!getToken(i).equalsTo(";")) { s+=getToken(i); i+=1; }
    		((SimpleNode) node).name_token = new Token(s);
	}
	/** Racine de l'arbre */
	else if(node instanceof ASTCompilationUnit) {
		((SimpleNode) node).visibility_token = new Token("");
		((SimpleNode) node).nodeType_token = new Token("Racine");
    		((SimpleNode) node).name_token = new Token("");
	}
	/** ClassBodyDeclaration || InterfaceMemberDeclaration */
	else if(node instanceof ASTClassBodyDeclaration || node instanceof ASTInterfaceMemberDeclaration) {
	    while(!(getToken(i).equalsTo("public")
		    || getToken(i).equalsTo("private")
		    || getToken(i).equalsTo("protected")
		    && i < 5))
		{
		    i+=1;
		}
	    if(i >= 4) {
		i=1;
		((SimpleNode) node).visibility_token = new Token("vide");
	    } else {
		((SimpleNode) node).visibility_token = getToken(i);
	    }
	    
	    while(getToken(i).equalsTo("static")
		  || getToken(i).equalsTo("final")
		  || getToken(i).equalsTo("native")
		  || getToken(i).equalsTo("private")
		  || getToken(i).equalsTo("protected")
		  || getToken(i).equalsTo("public")
		  || getToken(i).equalsTo("static")
		  || getToken(i).equalsTo("synchronized")
		  || getToken(i).equalsTo("strictfp")
		  || getToken(i).equalsTo("transient")
		  || getToken(i).equalsTo("volatile"))
		{
		    i+=1;
		}
	    j=i+1;
	    i=j-1;
	    if(getToken(i+2).equalsTo("(")) { // une methode
		((SimpleNode) node).return_token = getToken(i);
                ((SimpleNode) node).name_token = getToken(j);
                ((SimpleNode) node).nodeType_token = new Token("methode");
            } else if(getToken(i+1).equalsTo("(")) { // un constructeur
                ((SimpleNode) node).return_token = new Token("");
                ((SimpleNode) node).name_token = getToken(i);
                ((SimpleNode) node).nodeType_token = new Token("constructeur");
	    } else if(getToken(i+1).equalsTo("[")) { // un attribut de type tableau
		
		String s2 = getToken(i)+"";
		i+=1;
		while(getToken(i).equalsTo("[") || getToken(i).equalsTo("]")) {
		    s2+=getToken(i);
		    i+=1;
		}
		if(getToken(i+1).equalsTo("(")) { // une methode
		    ((SimpleNode) node).return_token = new Token(s2);
		    ((SimpleNode) node).name_token = getToken(i);
		    ((SimpleNode) node).nodeType_token = new Token("methode");
		} else {
		    ((SimpleNode) node).return_token = new Token(s2);
		    ((SimpleNode) node).name_token = getToken(i);
		    ((SimpleNode) node).nodeType_token = new Token("attribut");
		}
	    } else {//un attribut normal
		((SimpleNode) node).return_token = getToken(i);
		((SimpleNode) node).name_token = getToken(j);
		((SimpleNode) node).nodeType_token = new Token("attribut");

		if((((SimpleNode) node).return_token).equalsTo("class")) {
		    ((SimpleNode) node).return_token = new Token("class");
		    ((SimpleNode) node).nodeType_token = new Token("class");
		}
	    }
	}
	/** Sinon : les autres cas */
	else {
	    ((SimpleNode) node).visibility_token = new Token("");
	    ((SimpleNode) node).nodeType_token = new Token(JavaParserTreeConstants.jjtNodeName[((SimpleNode) node).getID()]);
	    ((SimpleNode) node).name_token = new Token("");
	}
}
    
  static final public void jjtreeCloseNodeScope( Node node ) { }
